/*
 * @(#) SoftMembrane.cpp   1.0   Dec 14, 2011
 *
...
SoftMembrane::SoftMembrane(b2World* world,
	...
	initSurface_ = radius * radius * 180.0f * deg2rad * 1.0;
	...
	bodyWidth_ = 180.0f * deg2rad * radius / nbrBodies;
	...
	for (int i = 1; i < (int) nbrBodies; ++i) {
		...
		revoluteJoint.Initialize(bodies_[i - 1], bodies_[i],
				bodies_[i - 1]->GetWorldCenter());
		this->joints_.push_back(world->CreateJoint(&revoluteJoint));

		if (i == (nbrBodies - 1)) {
			revoluteJoint.Initialize(bodies_[i], bodies_[0],
					bodies_[0]->GetWorldCenter());
			this->joints_.push_back(world->CreateJoint(&revoluteJoint));
		}
...
SoftMembraneDef& SoftMembrane::getDefinition() {
	return definition_;
}

// claculate the momentary surface and update the "initSurface_"
void SoftMembrane::updateSurface() {
	...
	//this->initSurface_ = abs(surface / 2) * 1.05;

}

float SoftMembrane::getSurface() {
	return this->initSurface_;
}

void SoftMembrane::setSurface(float surface) {
	this->initSurface_ = surface;
}

void SoftMembrane::computeDynamics(float globalFriction) {
...
std::vector<b2Body*>& SoftMembrane::getBodies() {
	return this->bodies_;
}

float SoftMembrane::getBodyLength() const {
	return this->bodyLength_;
}

float SoftMembrane::getBodyWidth() const {
	return this->bodyWidth_;
}

b2Vec2 SoftMembrane::getCenter() const {
...
void SoftMembrane::setCenter(b2Vec2 newBodyCenter) {
	this->oldBodyCenter_ = this->newBodyCenter_;
	this->newBodyCenter_ = newBodyCenter;
}

// get linear velocity
b2Vec2 SoftMembrane::getLinearVelocity(float delta_t) const {

	b2Vec2 linearVelocity;

	linearVelocity.x = (this->newBodyCenter_.x - this->oldBodyCenter_.x)
			/ delta_t;
	linearVelocity.y = (this->newBodyCenter_.y - this->oldBodyCenter_.y)
			/ delta_t;
...
void SoftMembrane::mergeMembrane(std::string mergeEdge, SoftMembrane* other,
