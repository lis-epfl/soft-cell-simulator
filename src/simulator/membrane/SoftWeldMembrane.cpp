...
SoftWeldMembrane::SoftWeldMembrane(b2World* world,
	...
	bodyWidth_ = (180.0f * deg2rad * radius / nbrBodies) * 0.95;
	...
	// Definition for weld joint between bodies
	b2WeldJointDef weldJoint;
	weldJoint.type = e_weldJoint;
	weldJoint.localAnchorA.Set(0.0f, 0.0f);
	weldJoint.localAnchorB.Set(0.0f, 0.0f);
	weldJoint.referenceAngle = 0.0f;
	weldJoint.frequencyHz = 0.0f;
	weldJoint.dampingRatio = 0.0f;
	...
	for (int i = 1; i < (int) nbrBodies; ++i) {
		...
		//weldJoint.Initialize(bodies_[i - 1], bodies_[i], bodies_[i - 1]->GetWorldCenter());
		weldJoint.bodyA = bodies_[i - 1];
		weldJoint.bodyB = bodies_[i];

		this->joints_.push_back(world->CreateJoint(&weldJoint));

		if (i == (nbrBodies - 1)) {
			weldJoint.bodyA = bodies_[i];
			weldJoint.bodyB = bodies_[0];
			this->joints_.push_back(world->CreateJoint(&weldJoint));
		}
...
SoftWeldMembrane::~SoftWeldMembrane() {
...
SoftMembraneDef& SoftWeldMembrane::getDefinition() {
	return definition_;
}

// Calculate the momentary surface and update the "initSurface_"
void SoftWeldMembrane::updateSurface() {
...
float SoftWeldMembrane::getSurface() {
	return this->initSurface_;
}

void SoftWeldMembrane::setSurface(float surface) {
	this->initSurface_ = surface;
}

void SoftWeldMembrane::computeDynamics(float globalFriction) {
...
std::vector<b2Body*>& SoftWeldMembrane::getBodies() {
	return this->bodies_;
}

float SoftWeldMembrane::getBodyLength() const {
	return this->bodyLength_;
}

float SoftWeldMembrane::getBodyWidth() const {
	return this->bodyWidth_;
}

b2Vec2 SoftWeldMembrane::getCenter() const {
...
void SoftWeldMembrane::mergeMembrane(std::string mergeEdge,
		SoftWeldMembrane* other, float elementsToConnect) {
